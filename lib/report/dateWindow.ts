import { startOfDay, toPayoutDate } from "@/lib/calendar/workingDays";
import {
...
export interface ReportCycle {
  ...
  incomeStart: Date;
  /** Для превью — с даты выплаты; для текущего — с сегодня. */
  expenseStart: Date;
...
export function resolveReportCycle(
  ...
  const expenseStart = isPreview ? payoutDate : todayStart;
...
/** Оба доступных цикла (к 10-му и к 25-му) относительно сегодня. */
export function listReportCycles(today: Date): ReportCycle[] {
  return [resolveReportCycle(today, 10), resolveReportCycle(today, 25)];
}
