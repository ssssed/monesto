...
export default function HomeScreen() {
  ...
  const loadReport = useCallback(async () => {
    ...
    const preferred =
      selectedDayRef.current ?? currentCycle?.paymentDay ?? availableCycles[0]?.paymentDay ?? 25;
  ...
  const handleSelectCycle = (day: SalaryPaymentDay) => {
  ...
  const handleConfirmAsset = async (assetId: number) => {
    ...
    if (newlyConfirmed.length === 0) {
      ...
      setConfirmedIds((prev) => [
        ...new Set([...prev, ...pending.map((p) => p.ruleId)]),
      ]);
      return;
    }

    await depositFromAllocation(
      assetId,
      totalRub,
      usdRubRate,
    ...
    return (
      <SafeAreaView className="flex-1 bg-white px-4 pt-6" edges={["top"]}>
        <Text className="text-base text-slate-700">
          {errorMessage ??
            "Не удалось построить отчёт. Проверьте доходы в настройках."}
        </Text>
      </SafeAreaView>
    );
  }
  ...
  const canSwipe = !report.isPreview;
  ...
  const payoutShifted =
    report.payoutDate.getTime() !== report.nominalDate.getTime();
        ...
        >
          <FadeInBlock>
            ...
            <Text className="mb-4 text-sm text-slate-500">
              ...
              {payoutShifted
                ? ` · выплата за ${report.nominalDate.getDate()}-е`
                : ""}
            </Text>
          </FadeInBlock>
          ...
          <FadeInItem index={2}>
            <View className="mb-5 flex-row gap-3">
              ...
              <View className="flex-1 rounded-2xl bg-red-50 p-4">
                <Text className="text-xs font-semibold uppercase text-red-600">
                  Расходы
                </Text>
          ...
          {assetsWithChanges.length === 0 ? (
            ...
            assetsWithChanges.map((item, index) => {
              ...
              const pending = related.filter(
                (a) =>
                  !confirmedIds.includes(a.ruleId) &&
                  !rejectedIds.includes(a.ruleId),
              );
              const allConfirmed =
                related.length > 0 &&
                related.every((a) => confirmedIds.includes(a.ruleId));
              const allRejected =
                related.length > 0 &&
              ...
              return (
                <FadeInItem key={item.id} index={index + 4}>
                  <SwipeConfirmCard
                    title={item.name}
                    balanceLabel={
                      item.provider === "usd"
                    ...
                    swipeable={canSwipe && pending.length > 0}
