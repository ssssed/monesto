...
/** Доля погашения 0…1. */
export function creditRepaidRatio(asset: AssetCalc): number | null {
  const initial = asset.goalAmount;
  if (initial == null || initial <= 0) return null;
  const repaid = Math.max(0, initial - asset.currentAmount);
  return Math.min(1, repaid / initial);
}

export function creditRepaidAmount(asset: AssetCalc): number {
  const initial = asset.goalAmount ?? asset.currentAmount;
  return Math.max(0, initial - asset.currentAmount);
}

export function hasCreditInterest(asset: AssetCalc): boolean {
  return (
    asset.provider === 'credit' &&
    asset.creditAnnualRate != null &&
    asset.creditAnnualRate > 0
  );
}
 ...
 *
 * `asOf` обязателен (в отличие от клиентской версии, которая по умолчанию
 * брала браузерное `new Date()`) — на сервере нет «локального сейчас»,
 * вызывающий код обязан явно передать разрешённую дату запроса.
 */
...
export function resolveRemainingMonthsForRecalc(
  ...
  schedule:
    | { startDate: string; termMonths: number; paymentDay: number; asOf: Date }
    | null
    | undefined,
): number | null {
  if (
...
export function creditMonthsLeft(
  ...
  if (annualPercent != null && annualPercent > 0) {
    return creditPayoffMonthsLeft(remainingDebt, monthlyPayment, annualPercent);
  }
...
export function creditScheduleContext(
  ...
  if (!asset.creditStartDate || !asset.creditTermMonths || !paymentDay) {
    return null;
  }
  return {
    startDate: asset.creditStartDate,
    termMonths: asset.creditTermMonths,
    paymentDay,
    asOf,
  };
}
...
export function buildSimpleClosingPlan(input: {
  ...
  const { remainingDebt, monthlyPayment, paymentDay, from, steps = 6 } = input;
...
export function applyEarlyRepayment(input: {
  ...
  if (input.mode === CreditEarlyRepayMode.reduce_payment) {
    ...
    return { newDebt, newPayment, newMonthsLeft: months, accrued, toPrincipal };
  }
...
export function previewEarlyRepayment(input: {
  ...
  const rate = input.asset.creditAnnualRate;
  if (rate == null || rate <= 0 || input.extraPayment <= 0) return null;
  if (
    input.monthlyPayment <= 0 &&
    input.mode === CreditEarlyRepayMode.reduce_term
  )
    return null;
  ...
  const currentPayoffMonths = creditPayoffMonthsLeft(
    input.asset.currentAmount,
    input.monthlyPayment,
    rate,
  );

  const months =
    input.mode === CreditEarlyRepayMode.reduce_payment
      ? resolveRemainingMonthsForRecalc(
          input.asset.currentAmount,
          input.monthlyPayment,
          rate,
          input.asset.creditRemainingMonths,
          schedule,
        )
      : null;
  ...
  const result = applyEarlyRepayment({
    remainingDebt: input.asset.currentAmount,
    extraPayment: input.extraPayment,
    monthlyPayment: input.monthlyPayment,
    annualPercent: rate,
    mode: input.mode,
  ...
  return { ...result, currentPayoffMonths, contractMonthsLeft };
}

export function resolveCreditPayment(
  asset: AssetCalc,
  expenses: ExpenseCalc[],
): { amount: number; dueDay: number | null; expense: ExpenseCalc | null } {
  const expense =
    asset.linkedExpenseId != null
      ? (expenses.find((e) => e.id === asset.linkedExpenseId) ?? null)
      : (expenses.find((e) => e.linkedAssetId === asset.id) ?? null);
  if (!expense) {
    return { amount: 0, dueDay: null, expense: null };
  }
  return { amount: expense.amount, dueDay: expense.dueDay, expense };
}
...
export function accruedInterestSinceDueDay(input: {
  ...
  const { remainingDebt, annualPercent, dueDay, onDate } = input;
  if (
    remainingDebt <= 0 ||
    annualPercent <= 0 ||
    dueDay == null ||
