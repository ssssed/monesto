...
import { countWorkingDaysExcludingVacation } from './vacation-pay';
import type {
  SalaryPaymentDay,
  SalaryTranche,
  VacationPeriodCalc,
...
export const DEFAULT_BIMONTHLY_TRANCHES: SalaryTranche[] = [
  { paymentDay: 10, periodFromDay: 16, periodToDay: 31, periodMonthOffset: -1 },
  { paymentDay: 25, periodFromDay: 1, periodToDay: 15, periodMonthOffset: 0 },
];

/** Альтернативный график: 5-е за 16–конец прошлого, 20-е за 1–15 текущего. */
export const PRESET_5_20_TRANCHES: SalaryTranche[] = [
  { paymentDay: 5, periodFromDay: 16, periodToDay: 31, periodMonthOffset: -1 },
  { paymentDay: 20, periodFromDay: 1, periodToDay: 15, periodMonthOffset: 0 },
];
...
export function getSalaryWorkPeriod(
  ...
  return { from, to, paymentDay: tranche.paymentDay };
}
...
export function calculateSalaryPaymentAmount(
  ...
  if (!tranche) {
    throw new Error(`Invalid salary payment day: no tranche for ${paymentDay}`);
  }
